    ...
    def save_generated_email(website, email_content):
        ...
        if website in existing_websites:
            ...
            return
        worksheet.append_row([website, email_content, "", ""])  # Added extra field for "Email found"
    ...
    def mark_as_sent(website):
        try:
            ...
            for idx, site in enumerate(websites):
                if site == website:
                    worksheet.update_cell(idx+1, 3, "Sent")  # Column 3 = Status
        ...
        try:
            ...
            for website in qualified_leads:
                ...
                if 'error' in scrape_resp or not scrape_resp.get('emails'):
                    continue  # Skip if no emails found
                ...
                # Save email + first found email address
